...
void COmxScriptParser::OnStartElementL(const RTagInfo& aElement,
	...
	if(!iInTest)
		...
		if(elemName == _L8("LoadComponent"))
			...
			HBufC* filename = HBufC::NewLC(filename8Bit.Length());
			filename->Des().Copy(filename8Bit);
			CheckForAbortL(iCallback.MosGetFilename(comp, *filename));
			CleanupStack::PopAndDestroy(filename);
			}
			...
			CheckForAbortL(iCallback.MosSetAudioPortDefL(comp, port, codingTypePtr));
			}
...
OMX_ERRORTYPE COmxScriptParser::ParseOmxErrorCode(const TDesC8& aErrorCode)
    ...
    if (aErrorCode == _L8("OMX_ErrorNone"))
        {
        error = OMX_ErrorNone;
        }
    else if (aErrorCode == _L8("OMX_ErrorBadPortIndex"))
        {
        error = OMX_ErrorBadPortIndex;
        }
    else if (aErrorCode == _L8("OMX_ErrorBadParameter"))
        {
        error = OMX_ErrorBadParameter;
        }
	else if (aErrorCode == _L8("OMX_ErrorPortsNotCompatible"))
		{
		error = OMX_ErrorPortsNotCompatible;
		}
	else if (aErrorCode == _L8("OMX_ErrorUnsupportedSetting"))
		{
		error = OMX_ErrorUnsupportedSetting;
		}
    else if (aErrorCode == _L8("OMX_ErrorIncorrectStateOperation"))
        {
        error = OMX_ErrorIncorrectStateOperation;
        }
    else if (aErrorCode == _L8("OMX_ErrorUnsupportedIndex"))
        {
        error = OMX_ErrorUnsupportedIndex;
        }
    else if (aErrorCode == _L8("OMX_ErrorNotReady"))
        {
        error = OMX_ErrorNotReady;
        }
    else if (aErrorCode == _L8("OMX_ErrorContentPipeOpenFailed"))
        {
        error = OMX_ErrorContentPipeOpenFailed;
        }
    else if (aErrorCode == _L8("OMX_ErrorNotImplemented"))
        {
