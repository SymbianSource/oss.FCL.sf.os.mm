...
COmxILClockOutputPort* COmxILClockOutputPort::NewL(const TOmxILCommonPortData& aCommonPortData, const RArray<OMX_OTHER_FORMATTYPE>& aSupportedFormats, COmxILClockProcessingFunction& aProcessingFunction)
	{
	COmxILClockOutputPort* self = new(ELeave) COmxILClockOutputPort(aCommonPortData, aProcessingFunction);
	CleanupStack::PushL(self);
	self->ConstructL(aSupportedFormats);
	CleanupStack::Pop(self);
	return self;
	}

COmxILClockOutputPort::COmxILClockOutputPort(const TOmxILCommonPortData& aCommonPortData, COmxILClockProcessingFunction& aProcessingFunction) :
 COmxILOtherPort(aCommonPortData),
 iProcessingFunction(&aProcessingFunction)
	{
	}

void COmxILClockOutputPort::ConstructL(const RArray<OMX_OTHER_FORMATTYPE>& aSupportedFormats)
	{
	COmxILOtherPort::ConstructL(aSupportedFormats);
	}
