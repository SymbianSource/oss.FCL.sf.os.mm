...
void COmxILGraphicSinkVPB0Port::ConstructL()
	...
	DEBUG_PRINTF(_L8("COmxILGraphicSinkVPB0Port::ConstructL +"));
 	...
 	iParamPortDefinition.format.video.pNativeRender = NULL;
 	iParamPortDefinition.format.video.pNativeWindow = NULL;
...
OMX_ERRORTYPE COmxILGraphicSinkVPB0Port::SetFormatInPortDefinition(const OMX_PARAM_PORTDEFINITIONTYPE& aPortDefinition,TBool& aUpdateProcessingFunction)
	...
	if (!UpdateColorFormat(iParamPortDefinition.format.video.eColorFormat, aPortDefinition.format.video.eColorFormat, aUpdateProcessingFunction))
		{
		return OMX_ErrorUnsupportedSetting;
		}

	if (!UpdateCodingType(iParamPortDefinition.format.video.eCompressionFormat, aPortDefinition.format.video.eCompressionFormat, aUpdateProcessingFunction))
		{
		return OMX_ErrorUnsupportedSetting;
		}

	if(iParamPortDefinition.format.video.nStride != aPortDefinition.format.video.nStride)
    	{
		...
		iParamPortDefinition.format.video.nStride = aPortDefinition.format.video.nStride;
		aUpdateProcessingFunction = ETrue;
    	}

	if (iParamPortDefinition.format.video.nFrameWidth != aPortDefinition.format.video.nFrameWidth ||
	    iParamPortDefinition.format.video.nFrameHeight != aPortDefinition.format.video.nFrameHeight ||
		iParamPortDefinition.format.video.nBitrate != aPortDefinition.format.video.nBitrate)
	    {
        iParamPortDefinition.format.video.nFrameWidth = aPortDefinition.format.video.nFrameWidth;
        iParamPortDefinition.format.video.nFrameHeight = aPortDefinition.format.video.nFrameHeight;
        // nSliceHeight is a RO attribute. Policy is to set it to the frame height
        iParamPortDefinition.format.video.nSliceHeight = aPortDefinition.format.video.nFrameHeight;
        iParamPortDefinition.format.video.nBitrate = aPortDefinition.format.video.nBitrate;    
        aUpdateProcessingFunction = ETrue;
	    }
...
OMX_ERRORTYPE COmxILGraphicSinkVPB0Port::SetParameter(OMX_INDEXTYPE aParamIndex,const TAny* apComponentParameterStructure, TBool& aUpdateProcessingFunction)
	...
	switch(aParamIndex)
		...
		case OMX_IndexParamVideoPortFormat:
			...
			if (iParamPortDefinition.format.video.xFramerate != componentParameterStructure->xFramerate)
				{
...
TBool COmxILGraphicSinkVPB0Port::IsTunnelledPortCompatible(const OMX_PARAM_PORTDEFINITIONTYPE& aPortDefinition) const
	...
	if (iParamPortDefinition.format.video.eColorFormat != aPortDefinition.format.video.eColorFormat)
		{
		return EFalse;
		}
    ...
    if (iParamPortDefinition.format.video.nFrameHeight != aPortDefinition.format.video.nFrameHeight)
        {
        return EFalse;
        }
...
void COmxILGraphicSinkVPB0Port::UpdateParamInPortDefinitionStruct()
	...
	iParamPortDefinition.format.video.eColorFormat = iParamVideoPortFormat.eColorFormat;
	iParamPortDefinition.format.video.xFramerate = iParamVideoPortFormat.xFramerate;
	iParamPortDefinition.format.video.eCompressionFormat = iParamVideoPortFormat.eCompressionFormat;
	}
...
OMX_ERRORTYPE COmxILGraphicSinkVPB0Port::ValidateStride()
	{
	if (iParamPortDefinition.format.video.nStride < iParamPortDefinition.format.video.nFrameWidth * BytesPerPixel(iParamPortDefinition.format.video.eColorFormat))
	    {
		return OMX_ErrorBadParameter;
		}
	return OMX_ErrorNone;
